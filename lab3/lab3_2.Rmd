...
```{r}
hot_springs <- readr::read_csv("hsprings_data.csv") ##readr is the package youre using
```

Use the `str()` function to get an idea of the data structure of `hot_springs`.  
```{r}
str(hot_springs) ##helps you get an idea of the structure of the csv file
```
...
```{r}
levels(hot_springs$scientist) ##since its a character it sees it as individual points
```


```{r}
hot_springs$scientist <- as.factor(hot_springs$scientist) ##changing what type of data "scientist" is classified as, this can be important because the difference between characters and factors can be very important

##factors have specific levels, vs characters have 
class(hot_springs$scientist)
```
...
```{r}
summary(fish) ##note that the treatment of NA data in summary functions can be sketchy, so this isnt a super great/accurate function
```
...
glimpse(mammals)
```
str shows the kinds of data present in each column (numerical or categorical, while glimpse does not)
