...
```{r}
x = filter(fish, between(scalelength, 2,4)) ##x= is unnecessary and is just done to run a check that the code we used it good (good as in it can be applied to other data)
```
...
```{r}
filter(fish, lakeid == "AL" | length >= 350) ## | means either, so youll get lake ids other than AL
```
...
```{r}
filter(fish, length > 400, (scalelength > 11 | radii_length_mm > 8)) ##remember, | means "or"
```
...
```{r}
primate_genera <- filter(mammals, genus $n$ c("Lophocebus", "Arythocebus", "Macaca"))
view(primate_genera)
```
