...
penguins %>% 
  count(island, sort = T) %>%
   #sort=T sorts the column in descending order, count is the same as group_by and summarise, but basically shorter
```
...
penguins %>%
  ...
  summarise(number_NAs=sum(is.na(sex)))%>%
  arrange(desc())
```
...
```{r}
penguins %>%
  summarise(across(!c(species, island, sex, bill_depth_mm, year), 
                   n_distinct))
```
